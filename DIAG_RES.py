...
#os.system('iptables -t mangle -A PREROUTING -j NFQUEUE --queue-num 2')
...
bind_layers(IP, DIAG_REQ)
bind_layers(DIAG_REQ, IP)
...
def cb(payload):
	...
	p = IP(data)
	if DIAG_REQ in p:
		print p[DIAG_REQ].SF_Map_Index
		if p[DIAG_REQ].SF_Map_Index==0: 
			try:
				sql = "SELECT SF FROM ServiceFunctions WHERE SF='%s'" % (p[DIAG_REQ].SF_ID)
				cursor.execute(sql)
